...
params = struct();
params.video = struct();
params.features = struct();

params.video.pixel_resolution_static = 0.09375; %degrees
params.video.pixel_resolution_motion = 0.09375; %degrees
params.video.contrast_max = 128; % for 8 bit image, positive or negative contrast

%feature extraction
params.features.gauss_filter_size_static = 1;
params.features.filter_gauss = 0.0;
params.features.OS_thres_frac = 0.5;
params.features.contrast_thres_frac = 0.1;
params.features.features_static = struct();
params.features.features_static.local_contrast = true;
params.features.features_static.horizontal_orientation = true;
params.features.features_static.vertical_orientation = true;
params.features.feature_spacing_motion = 10;
params.features.motion_feature_RF_center = 1;
params.features.motion_feature_RF_surround = 5;
params.features.motion_surround_weight = 1;
params.features.OMS_threshold_frac = 0.1;
params.features.loom_threshold_frac = 0.1;
params.features.LD_contrast_thres_frac = 0.1;
params.features.DVS_contrast_thres_frac = 0.1;
params.features.features_motion = struct();
params.features.features_motion.LD = true;
params.features.features_motion.OMS = true;
params.features.features_motion.DVS = true;

params.video.video_degrees_visual_angle = 120;
params.video.video_fname = ""; %filename of the video (full path if not in current directory)
params.video.save_fname = "";

data_dir = "/Users/shaysnyder/Downloads/feature-rich-spikes/data/berkeley_deep_drive/bdd100k/images/track/train";
sub_dirs = dir(data_dir);

for seq_dir_struct = sub_dirs'
    if strcmp(".", seq_dir_struct.name)
        continue
    elseif strcmp("..", seq_dir_struct.name)
        continue
    end

    video_path = fullfile( ...
        seq_dir_struct.folder, ...
        seq_dir_struct.name, ...
        "video.mp4" ...
    );
    ...
    params.video.video_fname = video_path;
    params.video.save_fname = feature_path;
    params.video.video_frames_path = fullfile( ...
        seq_dir_struct.folder, ...
        seq_dir_struct.name, ...
        "vidFrames.mat" ...
    );
    params.video.video_params_path = fullfile( ...
        seq_dir_struct.folder, ...
        seq_dir_struct.name, ...
        "vidFrames.mat" ...
    );
    params.video.feature_frames_path = fullfile( ...
        seq_dir_struct.folder, ...
        seq_dir_struct.name, ...
        "feature_frames.mat" ...
    );
    params.video.feature_params_path = fullfile( ...
        seq_dir_struct.folder, ...
        seq_dir_struct.name, ...
        "feature_params.mat" ...
    );
...
function v = feature_extractor(params)
...
%% Load video
if exist(params.video.video_frames_path, 'file')
    load(params.video.video_frames_path)
    fprintf('Loaded saved movie franes from %s\n', params.video.video_frames_path);
else
    ...
    save(params.video.video_frames_path, 'frames_static', 'frames_diff', ...
    ...
    save(params.video.video_params_path, 'video_params');
end
...
saved_feature_spikes_fname = params.video.feature_frames_path;
saved_feature_params_fname = params.video.feature_params_path;
